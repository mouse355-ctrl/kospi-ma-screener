...
class FakeProvider:
    ...
    def list_stocks(self, market, asof):
        return [
            StockInfo("000010", "상승주", 1e12),
            StockInfo("000020", "하락주", 2e12),
            StockInfo("000015", "상승주우", 3e12),   # 코드 끝자리 5 → 우선주, 제외
            StockInfo("069500", "KODEX 200", 5e12),  # ETF, 제외
            StockInfo("000040", "실패주", 4e12),
        ]
...
def _run(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("EXCLUDE_NON_STOCK", "true")
...
def test_json_output(tmp_path, monkeypatch):
    ...
    assert summary["total_screened"] == 3          # 우선주·ETF 제외
    assert summary["failed"] == 1
    assert summary["aligned_count"] == 1
    assert latest["results"][0]["code"] == "000010"
    assert latest["results"][0]["market_cap"] == 1e12
    assert (tmp_path / "history" / "2026-09-04.json").exists()
    prices = json.loads((tmp_path / "prices" / "000010.json").read_text(encoding="utf-8"))
    assert prices["columns"][0] == "date" and len(prices["bars"]) == 300
    assert prices["bars"][-1][0] == "2026-09-04"
    assert summary["excluded"] == {"PREFERRED": 1, "ETF": 1}
    assert not (tmp_path / "prices" / "000020.json").exists()
